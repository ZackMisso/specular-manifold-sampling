...
MTS_VARIANT std::atomic<int>
    SpecularManifoldSingleScatter<Float, Spectrum>::stats_solver_failed(0);
MTS_VARIANT std::atomic<int>
    SpecularManifoldSingleScatter<Float, Spectrum>::stats_solver_succeeded(0);
MTS_VARIANT std::atomic<int>
    SpecularManifoldSingleScatter<Float, Spectrum>::stats_bernoulli_trial_calls(
        0);
MTS_VARIANT std::atomic<int> SpecularManifoldSingleScatter<
    Float, Spectrum>::stats_bernoulli_trial_iterations(0);
MTS_VARIANT std::atomic<int> SpecularManifoldSingleScatter<
    Float, Spectrum>::stats_bernoulli_trial_iterations_max(0);

MTS_VARIANT
SpecularManifoldSingleScatter<Float, Spectrum>::SpecularManifoldSingleScatter(
...
SpecularManifoldSingleScatter<Float, Spectrum>::specular_manifold_sampling(
    const SurfaceInteraction3f &si, ref<Sampler> sampler) const {
    ...
    EmitterInteraction ei = SpecularManifold::sample_emitter_interaction(
        si, m_scene->caustic_emitters_single_scatter(), sampler);
    ...
    for (size_t shape_idx = 0; shape_idx < shapes.size(); ++shape_idx) {
        ...
        if (alpha > 0.f) {
            ...
            MicrofacetDistribution<Float, Spectrum> distr(
                MicrofacetType::Beckmann, alpha, alpha, false);
            std::tie(n_offset, p_offset) =
                distr.sample(Vector3f(0.f, 0.f, 1.f), sampler->next_2d());
        }

        Spectrum value(0.f);
        if (m_config.bernoulli) {
            // Unbiased SMS
            ...
            if (ei.is_directional()) {
                ...
                d *= rcp(dist);
                ray_vis = Ray3f(
                    ei.p, d, math::RayEpsilon<Float> * (1.f + hmax(abs(ei.p))),
            ...
            // Now estimate the (inverse) probability of this whole process with
            // Bernoulli trials
            Float inv_prob_estimate = 1.f;
            int iterations          = 1;
            stats_bernoulli_trial_calls++;
            while (true) {
                ScopedPhase scope_phase(
                    ProfilerPhase::SMSCausticsBernoulliTrials);

                auto [success_trial, si_final_trial, unused_trial] =
                    sample_path(specular_shape, si, ei, sampler, n_offset);
                Vector3f direction_trial = normalize(si_final_trial.p - si.p);
                if (success_trial && abs(dot(direction, direction_trial) -
                                         1.f) < m_config.uniqueness_threshold) {
                    break;
                }
                ...
                if (m_config.max_trials > 0 &&
                    ...
                    /* There is a tiny chance always to sample super weird paths
                       that will never occur again due to small numerical
                       imprecisions. So setting a (super conservative) threshold
                       here can help to avoid infinite loops. */
            ...
            for (int m = 0; m < m_config.max_trials; ++m) {
                ...
                for (size_t k = 0; k < solutions.size(); ++k) {
                    if (abs(dot(direction, solutions[k]) - 1.f) <
            ...
            // std::vector<Vector3f> all_solutions = std::vector<Vector3f>();
            // // TODO: need to do one sample

            // while (true) {
            //     std::vector<Vector3f> first_half_sols =
            //     std::vector<Vector3f>(); std::vector<Vector3f>
            //     second_half_sols =
            //         std::vector<Vector3f>();

            //     int n = all_solutions.size();
            //     for (int j = 0; j < n; ++j)
            //     {
            //         first_half_sols
            //     }
...
MTS_VARIANT
std::tuple<typename SpecularManifoldSingleScatter<Float, Spectrum>::Mask,
           typename SpecularManifoldSingleScatter<
               Float, Spectrum>::SurfaceInteraction3f,
           typename SpecularManifoldSingleScatter<
               Float, Spectrum>::SurfaceInteraction3f>
SpecularManifoldSingleScatter<Float, Spectrum>::sample_path(
    const ShapePtr shape, const SurfaceInteraction3f &si,
    const EmitterInteraction &ei, ref<Sampler> sampler,
    const Vector3f &n_offset, const Point3f &p_start) const {
    ...
    if (m_config.mnee_init) {
        ...
        if (reflection) {
            ...
            d_tmp = normalize(bbox.center() - si.p); // Modified MNEE for
                                                     // reflection.
        } else {
            // Standard MNEE
    ...
    if (m_config.twostage) {
        // Sample an offset normal from a gaussian defined by the LEAN
        // information, around the current offset normal
        Point2f mu_offset(-n_offset[0] / n_offset[2],
                          -n_offset[1] / n_offset[2]);
        auto [mu, sigma] = si_init.bsdf()->lean(si_init, true);
        Point2f slope    = SpecularManifold::sample_gaussian(
            0.5f * (mu + mu_offset), sigma, sampler->next_2d());
        Normal3f lean_normal_local =
            normalize(Normal3f(-slope[0], -slope[1], 1.f));

        // First run the solver on the smoothed version of the shape without
        // normal map. This will bring us close to the solutions on the actual
        // shape.
        auto [success_smooth, si_smooth] =
            newton_solver(si, vtx_init, ei, lean_normal_local, 1.f);
...
MTS_VARIANT Spectrum
SpecularManifoldSingleScatter<Float, Spectrum>::evaluate_path_contribution(
    const SurfaceInteraction3f &si, const EmitterInteraction &ei_,
    ...
    if (ei.is_point()) {
        ...
        ei.pdf            = ds.pdf;
        // Remove solid angle conversion factor. This is accounted for in the
        // geometric term.
    ...
    auto [sucess_e, vy] = SpecularManifold::emitter_interaction_to_vertex(
        m_scene, ei, vtx.p, si.time, si.wavelengths);
    if (!sucess_e) {
        return 0.f;
    }
    ...
    path_throughput *=
        specular_reflectance(si_final, normalize(ei.p - si_final.p));
...
MTS_VARIANT
std::pair<typename SpecularManifoldSingleScatter<Float, Spectrum>::Mask,
          typename SpecularManifoldSingleScatter<
              Float, Spectrum>::SurfaceInteraction3f>
SpecularManifoldSingleScatter<Float, Spectrum>::newton_solver(
    const SurfaceInteraction3f &si, const ManifoldVertex &vtx_init,
    const EmitterInteraction &ei, const Vector3f &n_offset,
    ...
    while (iterations < m_config.max_iterations) {
        ...
        if (m_config.halfvector_constraints) {
            ...
            std::tie(step_success, C, dX) =
                compute_step_halfvector(si.p, vtx, ei, n_offset);
        } else {
            // Use angle-difference constraint formulation
            std::tie(step_success, C, dX) =
                compute_step_anglediff(si.p, vtx, ei, n_offset);
        }
        if (!step_success) {
            break;
        }
        ...
        // Make a proposal
        Vector3f p_prop = vtx.p - m_config.step_scale * beta *
                                      (vtx.dp_du * dX[0] + vtx.dp_dv * dX[1]);
        ...
        if (!si_current.is_valid() || vtx.shape != si_current.shape) {
            // Missed scene completely or hit different shape, need smaller
            // step!
            beta = 0.5f * beta;
            iterations += 1;
            continue;
    ...
    Float cos_theta_x = dot(vtx.gn, wx), cos_theta_y = dot(vtx.gn, wy);
...
MTS_VARIANT std::tuple<
    typename SpecularManifoldSingleScatter<Float, Spectrum>::Mask,
    typename SpecularManifoldSingleScatter<Float, Spectrum>::Vector2f,
    typename SpecularManifoldSingleScatter<Float, Spectrum>::Vector2f>
SpecularManifoldSingleScatter<Float, Spectrum>::compute_step_halfvector(
    const Point3f &v0p, const ManifoldVertex &v1, const EmitterInteraction &v2,
    ...
    Float ilo = norm(wo);
    if (ilo < 1e-3f) {
        return std::make_tuple(false, Vector2f(math::Infinity<Float>),
                               Vector2f(0.f));
    }
    ilo = rcp(ilo);
    wo *= ilo;
...
MTS_VARIANT std::tuple<
    ...
    typename SpecularManifoldSingleScatter<Float, Spectrum>::Vector2f,
    typename SpecularManifoldSingleScatter<Float, Spectrum>::Vector2f>
SpecularManifoldSingleScatter<Float, Spectrum>::compute_step_anglediff(
    const Point3f &v0p, const ManifoldVertex &v1, const EmitterInteraction &v2,
    const Vector3f &n_offset) const {
    ...
    auto d_transform = [&](const Vector3f &w, const Vector3f &dw_du,
                           ...
                           Float eta) {
        if (eta == 1.f) {
            return SpecularManifold::d_reflect(w, dw_du, dw_dv, n, dn_du,
                                               dn_dv);
        } else {
            return SpecularManifold::d_refract(w, dw_du, dw_dv, n, dn_du, dn_dv,
                                               eta);
        }
    };

    // Handle offset normal. These are no-ops in case n_offset=[0,0,1]
    Normal3f n = v1.s * n_offset[0] + v1.t * n_offset[1] + v1.n * n_offset[2];
    Vector3f dn_du = v1.ds_du * n_offset[0] + v1.dt_du * n_offset[1] +
                     v1.dn_du * n_offset[2];
    Vector3f dn_dv = v1.ds_dv * n_offset[0] + v1.dt_dv * n_offset[1] +
                     v1.dn_dv * n_offset[2];
    ...
    if (valid_i_refr_i) {
        auto [dwio_du, dwio_dv] =
            d_transform(wi, dwi_du, dwi_dv, n, dn_du, dn_dv, v1.eta);
        ...
        auto [dto_du, dpo_du, dto_dv, dpo_dv] =
            SpecularManifold::d_sphcoords(wo, dwo_du, dwo_dv);
        auto [dtio_du, dpio_du, dtio_dv, dpio_dv] =
            SpecularManifold::d_sphcoords(wio, dwio_du, dwio_dv);
    ...
    if (valid_o_refr_o && !success) {
        auto [dwoi_du, dwoi_dv] =
            d_transform(wo, dwo_du, dwo_dv, n, dn_du, dn_dv, v1.eta);
        ...
        auto [dti_du, dpi_du, dti_dv, dpi_dv] =
            SpecularManifold::d_sphcoords(wi, dwi_du, dwi_dv);
        auto [dtoi_du, dpoi_du, dtoi_dv, dpoi_dv] =
            SpecularManifold::d_sphcoords(woi, dwoi_du, dwoi_dv);
...
MTS_VARIANT Spectrum
SpecularManifoldSingleScatter<Float, Spectrum>::specular_reflectance(
    const SurfaceInteraction3f &si, const Vector3f &wo) const {
    if (!si.is_valid())
        return 0.f;
    ...
    if (bsdf->roughness() > 0.f) {
        ...
        /* Compared to Eq. 6 in [Hanika et al. 2015 (MNEE)], two terms are
           omitted: 1) abs_dot(wo, n) is part of BSDF::eval
           2) abs_dot(h, n)  is part of the Microfacet distr. (also in
           BSDF::eval) */
        Vector3f h_l;
        if (reflection) {
            ...
            bsdf_val *= sqr(dot(si.wi, h_l) + eta * dot(wo_l, h_l)) /
                        (eta * eta * abs_dot(wo_l, h_l));
        }
    } else {
...
MTS_VARIANT Float
SpecularManifoldSingleScatter<Float, Spectrum>::geometric_term(
    const ManifoldVertex &v0, const ManifoldVertex &v1,
    const ManifoldVertex &v2) const {
    // A lot of the computation here overlaps with 'compute_step_halfvector',
    // but we need to do a bit more work involving the endpoints to get the full
    // geometric term.
    ...
    if (v2.fixed_direction) {
        ...
        dc2_dx1 = Matrix2f(dot(dc2_du1, v2.dp_du), dot(dc2_dv1, v2.dp_du),
        ...
        dc2_dx2 = Matrix2f(dot(dc2_du2, v2.dp_du), dot(dc2_dv2, v2.dp_du),
    ...
    Float dot_dpdu_n = dot(v1.dp_du, v1.n), dot_dpdv_n = dot(v1.dp_dv, v1.n);
    Vector3f s = v1.dp_du - dot_dpdu_n * v1.n, t = v1.dp_dv - dot_dpdv_n * v1.n;
    ...
    if (v2.fixed_direction) {
        ...
        dc1_dx0 = Matrix2f(dot(dh_du, s), dot(dh_dv, s), dot(dh_du, t),
                           dot(dh_dv, t));
    }
    ...
    Float dot_h_n = dot(h, v1.n), dot_h_dndu = dot(h, v1.dn_du),
          dot_h_dndv = dot(h, v1.dn_dv);
    Matrix2f dc1_dx1(dot(dh_du, s) - dot(v1.dp_du, v1.dn_du) * dot_h_n -
                         dot_dpdu_n * dot_h_dndu,
                     dot(dh_dv, s) - dot(v1.dp_du, v1.dn_dv) * dot_h_n -
                         dot_dpdu_n * dot_h_dndv,
                     dot(dh_du, t) - dot(v1.dp_dv, v1.dn_du) * dot_h_n -
                         dot_dpdv_n * dot_h_dndu,
                     dot(dh_dv, t) - dot(v1.dp_dv, v1.dn_dv) * dot_h_n -
    ...
    Matrix2f dc1_dx2(dot(dh_du, s), dot(dh_dv, s), dot(dh_du, t),
                     dot(dh_dv, t));
    ...
    if (v2.fixed_direction) {
        ...
        G = min(G, Float(10.f));
        G /= abs_dot(wi, v0.n); // Cancel out cosine term that will be added
                                // during BSDF evaluation
        G *= sqr(eta);
        return G;
    } else {
...
MTS_VARIANT void
SpecularManifoldSingleScatter<Float, Spectrum>::print_statistics() {
    Float solver_success_ratio = Float(stats_solver_succeeded) /
                                 (stats_solver_succeeded + stats_solver_failed),
          solver_fail_ratio = Float(stats_solver_failed) /
                              (stats_solver_succeeded + stats_solver_failed);
    ...
    std::cout << std::setw(25) << std::left
              << "Solver succeeded: " << std::setw(10) << std::right
              << stats_solver_succeeded << " " << std::setw(8) << "("
              << 100 * solver_success_ratio << "%)" << std::endl;
    std::cout << std::setw(25) << std::left
              << "Solver failed: " << std::setw(10) << std::right
              << stats_solver_failed << " " << std::setw(8) << "("
              << 100 * solver_fail_ratio << "%)" << std::endl;
    std::cout << std::endl;

    Float stats_booth_avg_iterations =
        Float(stats_bernoulli_trial_iterations) / stats_bernoulli_trial_calls;
    std::cout << std::setw(25) << std::left
              << "avg. Booth iterations: " << std::setw(10) << std::right
              << stats_booth_avg_iterations << std::endl;
    std::cout << std::setw(25) << std::left
              << "max. Booth iterations: " << std::setw(10) << std::right
              << stats_bernoulli_trial_iterations_max << std::endl;
    std::cout << std::endl;
    std::cout << "----------------------------------------------------------"
              << std::endl;
    std::cout << std::endl;
