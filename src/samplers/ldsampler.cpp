...
---------------------------------------------
This plugin implements a simple hybrid sampler that combines aspects of a
Quasi-Monte Carlo sequence with a pseudorandom number generator based on a
technique proposed by Kollig and Keller :cite:`Kollig2002Efficient`. It is a
good and fast general-purpose sample generator. Other QMC samplers exist that
can generate even better distributed samples, but this comes at a higher cost in
terms of performance. This plugin is based on Mitsuba 1's default sampler (also
called :monosp:`ldsampler`). Roughly, the idea of this sampler is that all of
the individual 2D sample dimensions are first filled using the same (0,
2)-sequence, which is then randomly scrambled and permuted using a shuffle
network. The name of this plugin stems from the fact that, by construction, (0,
2)-sequences achieve a low `star discrepancy
<https://en.wikipedia.org/wiki/Low-discrepancy_sequence>`_, which is a quality
criterion on their spatial distribution.
.. subfigstart::
.. subfigure::
../../resources/data/docs/images/render/sampler_independent_16spp.jpg :caption:
Independent sampler - 16 samples per pixel
.. subfigure::
../../resources/data/docs/images/render/sampler_ldsampler_16spp.jpg :caption:
Low-discrepancy sampler - 16 samples per pixel
.. subfigend::
   :label: fig-ldsampler-renders
.. subfigstart::
.. subfigure::
../../resources/data/docs/images/sampler/ldsampler_1024_samples.svg :caption:
1024 samples projected onto the first two dimensions.
.. subfigure::
../../resources/data/docs/images/sampler/ldsampler_64_samples_and_proj.svg
   :caption: A projection of the first 64 samples onto the first two dimensions
and their projection on both 1D axis (top and right plot). The 1D stratification
is perfect as this sampler doesn't add additional random perturbation to the
sample positions.
.. subfigend::
   :label: fig-ldsampler-pattern
.. subfigstart::
.. subfigure::
../../resources/data/docs/images/sampler/ldsampler_1024_samples_dim_32.svg
   :caption: 1024 samples projected onto the 32th and 33th dimensions, which
look almost identical. However, note that the points have been scrambled to
reduce correlations between dimensions.
.. subfigure::
../../resources/data/docs/images/sampler/ldsampler_64_samples_and_proj_dim_32.svg
   :caption: A projection of the first 64 samples onto the 32th and 33th
dimensions.
.. subfigend::
