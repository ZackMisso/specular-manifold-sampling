...
// can generate even better distributed samples, but this comes at a higher cost
// in terms of performance. This plugin is based on Mitsuba 1's default sampler
// (also called :monosp:`ldsampler`). Roughly, the idea of this sampler is that
// all of the individual 2D sample dimensions are first filled using the same
// (0, 2)-sequence, which is then randomly scrambled and permuted using a
// shuffle network. The name of this plugin stems from the fact that, by
// construction, (0, 2)-sequences achieve a low `star discrepancy
// <https://en.wikipedia.org/wiki/Low-discrepancy_sequence>`_, which is a
// quality criterion on their spatial distribution.
// .. subfigstart::
// .. subfigure::
// ../../resources/data/docs/images/render/sampler_independent_16spp.jpg
...
//    :caption: A projection of the first 64 samples onto the first two
//    dimensions
// and their projection on both 1D axis (top and right plot). The 1D
// stratification is perfect as this sampler doesn't add additional random
// perturbation to the sample positions.
// .. subfigend::
