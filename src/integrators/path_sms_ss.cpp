 ...
 * scenes currently. Extensions such as (approximative) MIS to robustly
 * transition to classical sampling strategies in cases of high roughness or
 * large light sources are an interesting direction for future work.
 *
 *
...
class SingleScatterSMSPathIntegrator
    : public MonteCarloIntegrator<Float, Spectrum> {
public:
    MTS_IMPORT_BASE(MonteCarloIntegrator, m_max_depth, m_rr_depth)
    MTS_IMPORT_TYPES(Scene, Sampler, Sensor, Emitter, EmitterPtr, BSDF, BSDFPtr,
                     ShapePtr, Medium)
    using SpecularManifold = SpecularManifold<Float, Spectrum>;
    using SpecularManifoldSingleScatter =
        SpecularManifoldSingleScatter<Float, Spectrum>;
    ...
    SingleScatterSMSPathIntegrator(const Properties &props) : Base(props) {
        ...
        m_sms_config.blanchet_pmf_var = props.bool_("pmf_var", false);
        m_sms_config.blanchet_cmf     = props.bool_("blanchet_cmf", false);
        m_sms_config.blanchet_ind     = props.bool_("blanchet_ind", false);
        m_sms_config.blanchet_n       = props.int_("blanchet_n", 0);
        ...
        m_disable_reflected_emitters =
            props.bool_("disable_reflected_emitters", false);
        m_biased_mnee = props.bool_("biased_mnee", false);
    }

    bool render(Scene *scene, Sensor *sensor) override {
        bool result =
            MonteCarloIntegrator<Float, Spectrum>::render(scene, sensor);
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene, Sampler *sampler,
        ...
        if constexpr (is_array_v<Float>) {
            Throw(
                "This integrator does not support vector/gpu/autodiff modes!");
            return { 0.f, 0.f };
        } else {
            ...
            for (int depth = 1;; ++depth) {
                ...
                if (si.shape->is_caustic_receiver() && !on_caustic_caster &&
                    ...
                    result +=
                        throughput * mf.specular_manifold_sampling(si, sampler);
                }
                ...
                /* As usual, emitter sampling only makes sense on Smooth BSDFs
                   ...
                   casters---with the exception of the first bounce where we
                   might see a direct (glossy) reflection of a light source this
                   way.
                ...
                if (has_flag(bsdf->flags(), BSDFFlags::Smooth) &&
                        !on_caustic_caster ||
                    (depth == 1 && !m_disable_reflected_emitters)) {
                    ...
                    auto [ds, emitter_weight] = scene->sample_emitter_direction(
                ...
                auto [bs, bsdf_weight] = bsdf->sample(
                ...
                if (emitter) {
                    ...
                    if (!on_caustic_caster ||
                        (depth == 1 && !m_disable_reflected_emitters)) {
                        /* Only do BSDF sampling in usual way if we don't
                           interact with a caustic caster now. */
                        ...
                        result += mis * throughput * emitter_val;
                               ...
                               si_prev.is_valid() &&
                               si_prev.shape->is_caustic_receiver() &&
                               emitter->is_caustic_emitter_single_scatter()) {
                        /* These are the light paths that can be sampled with
                           SMS. In case we're doing MNEE, only a single
                           deterministic path can be generated though, and if we
                           wish to stay unbiased we need to do an additional
                           test here to see if MNEE could generate the currently
                           found light connection as well. Note: Hanika et al.
                           2015 discuss a more advanced MIS strategy here that
                           also accounts for the smooth probablility density
                           from rough BSDFs. This could be added as well here.
                         */
                        ShapePtr specular_shape = si.shape;
                        EmitterInteraction ei =
                            SpecularManifold::emitter_interaction(
                                scene, si_prev, si_bsdf);

                        SpecularManifoldSingleScatter mf(scene, m_sms_config);
                        auto [success, si_mnee, unused1] = mf.sample_path(
                            specular_shape, si_prev, ei, sampler);
                        if (success) {
                            Vector3f direction_pt = normalize(si.p - si_prev.p),
                                     direction_mnee =
                                         normalize(si_mnee.p - si_prev.p);
                            if (abs(dot(direction_pt, direction_mnee) - 1.f) >=
                                m_sms_config.uniqueness_threshold) {
                                success = false;
                            }
                        }
                        ...
                        if (!success) {
                            /* MNEE could not find this path, so add it now.
                               There is no MIS needed as we filtered out this
                               class of paths in the emitter sampling strategy
                               above. Note that the original paper about MNEE is
                               more thorough here and does full MIS which
                               improves the case of caustics from rough BSDFs.
                               For simplicity we leave this out, but it could be
                               added as well. In that case, we would also need
                               to perform this "MNEE check" in the emitter
                               sampling step above. */
...
protected:
    ...
    bool m_disable_reflected_emitters; // Potentially disable directly reflected
                                       // emitters / "glints"
    bool m_biased_mnee; // Make MNEE biased by filtering out caustic paths that
                        // can't be sampled with it
};

MTS_IMPLEMENT_CLASS_VARIANT(SingleScatterSMSPathIntegrator,
                            MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(SingleScatterSMSPathIntegrator,
                  "Single-Bounce SMS Path Tracer integrator");
