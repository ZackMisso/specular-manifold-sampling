...
public:
     ...
     * Hammersley sequences. Roughly, it computes a b-ary representation of the
     * input value \c index, mirrors it along the decimal point, and returns the
     * resulting fractional value. The implementation here uses prime numbers
     * for \c b.
     *
     * \param base_index
     *     Selects the n-th prime that is used as a base when computing the
     * radical inverse function (0 corresponds to 2, 1->3, 2->5, etc.). The
     * value specified here must be between 0 and 1023.
     *
     * \param index
    ...
    Float eval(size_t base_index, UInt64 index) const {
        ...
        UInt64 value(zero<UInt64>()), divisor((uint64_t) base.value);
        Float factor = Float(1.f), recip(base.recip);
    ...
    Float eval_scrambled(size_t base_index, UInt64 index) const {
        ...
        UInt64 value(zero<UInt64>()), divisor((uint64_t) base.value),
            mask(0xffffu);
        Float factor(1.f), recip(base.recip);
        ...
        while (any(active)) {
            ...
            masked(value, active) =
                value * divisor +
                (enoki::gather<UInt64, 2>(perm, digit, active) & mask);
            index  = next;
            active = neq(index, enoki::zero<UInt64>());
        }

        Float correction(base.recip * (Float) perm[0] /
                         ((Float) 1 - base.recip));
        return min(math::OneMinusEpsilon<Float>,
                   (Float(value) + correction) * factor);
    }

    /// Return the permutation corresponding to the given prime number basis
    uint16_t *permutation(size_t basis) const { return m_permutations[basis]; }
...
Float radical_inverse_2(UInt index, UInt scramble = 0) {
    if constexpr (is_double_v<Float>) {
        ...
        index = ((index & 0x0000ffff0000ffffULL) << 16) |
                ((index & 0xffff0000ffff0000ULL) >> 16);
        index = ((index & 0x00ff00ff00ff00ffULL) << 8) |
                ((index & 0xff00ff00ff00ff00ULL) >> 8);
        index = ((index & 0x0f0f0f0f0f0f0f0fULL) << 4) |
                ((index & 0xf0f0f0f0f0f0f0f0ULL) >> 4);
        index = ((index & 0x3333333333333333ULL) << 2) |
                ((index & 0xccccccccccccccccULL) >> 2);
        index = ((index & 0x5555555555555555ULL) << 1) |
                ((index & 0xaaaaaaaaaaaaaaaaULL) >> 1);

        /* Generate an uniformly distributed double precision number in [1,2)
         * from the scrambled index and subtract 1. */
        return reinterpret_array<Float>(sr<12>(index ^ scramble) |
         ...
         * from the scrambled index and subtract 1. */
        return reinterpret_array<Float>(sr<9>(index ^ scramble) | 0x3f800000u) -
               1.f;
    }
}
...
Float sobol_2(UInt index, UInt scramble = 0) {
    if constexpr (is_double_v<Float>) {
        ...
        return reinterpret_array<Float>(sr<12>(scramble) |
                                        0x3ff0000000000000ull) -
               1.0;
    } else {
